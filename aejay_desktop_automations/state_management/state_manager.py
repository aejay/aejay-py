"""
This module contains the StateManager class, which is responsible for connecting to the MQTT broker
and receiving updates to the state of the machine.
"""
from typing import Callable, Union
import ssl
import sys
from paho.mqtt import client as mqtt
from .state_records import RemoteState

# only import pyobjc if on macOS:
if sys.platform == "darwin":
    from Foundation import NSObject, NSDistributedNotificationCenter, NSLog
    from PyObjCTools import AppHelper


class StateManager:
    """
    A type that manages the state of the machine by connecting to the MQTT broker and receiving updates
    ...
    def _on_connect(
        ...
        flags: dict[str, int],
        rc: mqtt.ReasonCodes,
        _properties: Union[mqtt.Properties, None],
    ) -> None:
        if rc == 0:
